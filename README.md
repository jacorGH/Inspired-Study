...
- **Presenting an outline live** — once it's shared, the leader gets a presenting bar (▶ Start, then ‹ Prev / Next ›) at the top of it. Stepping through highlights the current block and, for anyone with *Follow along* on, jumps them straight there — reusing the exact toggle that already follows Bible chapters, rather than adding a second one to keep track of. Position is tracked by the block's own id rather than its place in the list, so inserting or reordering blocks mid-session doesn't point the highlight at the wrong thing afterward.
